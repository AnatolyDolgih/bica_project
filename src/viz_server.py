...
headers = {
    'Authorization': 'x_cTiM9n_TLr80E85jjr4fSm4OfTTL8m3GyG_UOvoRLf7ajZDKvpEMgTDsQtacRwjoniI3qfaUKpJ-2lOEFY1g',
}
...
messages = [ {"role": "system", "content":start_promt} ]
...
def process_dialog(item: TextItem):
    string = "Hello"
    return {"answer_text" : string}

@app.post("/getAnswerEssay")
def process_essay(item: TextItem):
    string = "Hello"
    return {"output" : string}

@app.post("/getAnswer")
def read_post(item: PostItem):
...
@app.post("/getEmotion")
def read_post1(item: EmotionInput):
    print(f"direction: {item.direction}")
    return {
            "Desk": "",
            "Reply": "Тест реплика для озвучивания",
            "Direction": "player",
            "Happy": 0.1,
            "Sad": 0.05,
            "Surprise": 1,
